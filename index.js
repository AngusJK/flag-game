...
rl.question("Welcome to Guess The Flag. In this game you will be shown ten(10) country flags and you must correctly identify them. Type 'play' to continue.\n", (answer1) => {
  if (answer1 === "play") {
    ...
    const poseQuestion = function() {
      let index = Math.floor(Math.random() * 193);
      let randomCountryName = keys[index];
      let randomFlag = flags[`${randomCountryName}`]["flag"];
      let option1 = flags[keys[Math.floor(Math.random() * 193)]]["name"];
      let option2 = flags[keys[Math.floor(Math.random() * 193)]]["name"];
      let option3 = flags[keys[Math.floor(Math.random() * 193)]]["name"];
      let option4 = flags[keys[Math.floor(Math.random() * 193)]]["name"];
